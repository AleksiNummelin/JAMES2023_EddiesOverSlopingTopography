...
! ------------------------------------------------------------------------------
   ...
   logical :: &
      edsprs, & ! If true, apply eddy mixing suppression away from steering
                ! level.
      anisot,&  ! If true, apply anisotropy correction to diffusivity
      redi3D,&  ! If true, then isopycnal diffusion will have 3D structure
                ! based in the 3D structure of anisotropy.
      rhisctp, &! If true, use the minimum of planetary and topographic beta
                ! to define the Rhines scale.
      rhisctptw ! If true, use the minimum of planetary and topographic beta
                ! and thermal wind to define the Rhines scale.
   character(len = 80) :: &
   ...
   public :: egc, rhiscf, eggam, eglsmn, egmndf, egmxdf, egidfq, ri0, bdmc1, & 
             bdmc2, tkepf, bdmtyp, edsprs, rhisctp, rhisctptw, eitmth, edritp, &
             edwmth, difint, difiso, difdia, difmxp, difmxq, difwgt, &
             umfltd, vmfltd, utfltd, vtfltd, utflld, vtflld, &
             usfltd, vsfltd, usflld, vsflld, &
             inivar_diffusion, btpfile, anisot, redi3D
